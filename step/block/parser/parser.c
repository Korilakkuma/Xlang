...
struct Node *statement(void) {
  ...
  if (test_operator_and_next("if")) {
    node = create_node(NODE_IF, NULL, NULL);

    operator_and_next("(");

    node->condtion = expression();
  ...
  if (test_operator_and_next("while")) {
    node = create_node(NODE_WHILE, NULL, NULL);
  ...
  if (test_operator_and_next("for")) {
    node = create_node(NODE_FOR, NULL, NULL);

    operator_and_next("(");

    node->init_statement = expression();
    ...
    node->update_statement = expression();
